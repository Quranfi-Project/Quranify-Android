...
const ReadingMode = () => {
  ...
  const goToPrev = () => {
    if (currentPage > 1) navigate(`/read/${currentPage - 1}`);
  };

  const goToNext = () => {
    if (currentPage < TOTAL_PAGES) navigate(`/read/${currentPage + 1}`);
  };
      ...
      <div className="max-w-2xl mx-auto px-4 pt-6 pb-32">
        ...
        {error && (
          <div className="text-center mt-16 text-red-500 dark:text-red-400">
            <p>{error}</p>
            <button
              onClick={() => navigate(`/read/${currentPage}`)}
