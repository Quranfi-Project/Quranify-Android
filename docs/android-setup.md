...
Run `npm run build` first. Vite outputs to `dist/` which is what `webDir` in `capacitor.config.ts` points to.
...
**Network requests blocked on Android**
The app calls external APIs (alquran.cloud, qurancdn.com, mp3quran.net, quran.com) directly from the Capacitor WebView. If requests are blocked, check that `android/app/src/main/res/xml/network_security_config.xml` allows cleartext or that all target URLs use HTTPS (they do).
